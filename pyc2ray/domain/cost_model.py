...
class CostModel(ABC):
    """ Abstract class providing a common interface for memory/computational cost models associated
     ...
     Attributes
     ----------
     max_cost_per_group : float
        Maximum (model dependent) allowed cost for a source group, used as a threshold in grouping algorithms.
    """
...
class pyC2RayCostModel(CostModel):
    ...
    """
    def __init__(self, max_memory_cost_per_group: float, source_batch_size: int, is_periodic_mode_active: bool, 
                 photo_ion_table_size: int) -> None:
    ...
    def _compute_cells_to_shell(self, R: float, n_cells_per_side: int) -> int:
        ...
        ----------
        R : float
            Maximum radius of source influence in grid units.
        n_cells_per_side : int
            Number of grid cells per side in the local grid.
    ...
    def compute_group_costs(self, R: float, n_cells_per_side: int, n_src: int) -> tuple[float, float]:
        ''' Compute both memory and computational costs of processing a source group.
        ...
        mem_cost = 8 * (3 * n_cells + self.source_batch_size * cells_to_shell + 2 * self.photo_ion_table_size + n_src) + 12 * n_src

        if self.is_periodic_mode_active == True:
            comp_cost = n_src * min(n_cells, cells_to_shell)
            return mem_cost, comp_cost
        else:
            raise NotImplementedError("Computational cost model for non-periodic mode is not implemented.")