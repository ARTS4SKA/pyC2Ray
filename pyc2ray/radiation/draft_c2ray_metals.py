...
class C2Ray_Metals(C2Ray):
    ...
    def _radiation_init(self):
        """Build the BPASS cache instead of a single SED source."""
        # Pull tau-grid params and other photo settings from YAML, same as base class
        ...
        # Build the tau grid using the existing helper
        from pyc2ray.radiation import make_tau_table
        ...
        # New BPASS-specific config — add these to your YAML
        sed_files = self._ld["BPASS"]["sed_files"]      # dict {Z: path}
        file_ages = self._ld["BPASS"]["file_ages"]      # full BPASS age grid in Myr
        sim_ages  = self._ld["BPASS"]["sim_ages"]       # e.g. [1.0, 2.0, ..., 10.0]
    
        self.printlog("Building BPASS table cache…")
        self.bpass_cache = BPASSTableCache(
            sed_files=sed_files,
            sim_ages=sim_ages,
            file_ages=file_ages,
            tau=self.tau,
        ...
        self.Z_bin_centers = self.bpass_cache.Z_bin_centers
        self.printlog(f"Cache built: {self.bpass_cache!r}, {self.bpass_cache.memory_mb():.2f} MB")
