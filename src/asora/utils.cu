...
namespace {
    ...
    __host__ __device__ int2 divmod(int x, int y) { return {x / y, x % y}; }
...
namespace asora {
    ...
    __host__ __device__ int3 linthrd2cart(int q, int s) {
    ...
    __host__ __device__ int2 cart2linthrd(int i, int j, int k) {
    ...
    // Inverse of cart2shell, with the shell index q known.
    ...
    // Forward mapping: nothing to invert, just count the layers below and step along
    // the ring, two slots per j plus one more for the positive k twin.
