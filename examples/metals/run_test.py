...
for k in range(i_start, len(zred_array)-1):
    ...
    # Read input files
    sim.read_density(fbase='CDM_100Mpc_2048.%05d.ovrden.npy' %idx_zred[k], z=zi)

    # Read source files
    srcpos, normflux = sim.ionizing_flux(file='CDM_100Mpc_2048.%05d.halo.txt' %idx_zred[k], z=zi, dt=dt) #, save_Mstar=sim.results_basename+'/sources')
