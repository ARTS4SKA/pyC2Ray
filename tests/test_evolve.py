...
def setup_evolve_mock(use_gpu: bool = False, rank: int = 0):
    ...
    minltau, maxltau, num_tau = -20.0, 4.0, 20000
    _, dltau = make_tau_table(minltau, maxltau, num_tau)
    logtau = minltau, dltau, num_tau
    sigma = 6.3e-18
...
def setup_evolve_asora(
    ...
    # Calculate the table
    T_eff = 5e4
    radsource = BlackBodySource_Multifreq(T_eff)
    ...
    ndens = np.pow(10, rng.uniform(-6, -4, size=mesh_shape), dtype=np.float64)

    xHII = np.full_like(ndens, 1.0e-20)
    xHeII = np.full_like(ndens, 1.0e-20)
    xHeIII = np.full_like(ndens, 1.0e-20)

    # temperature [K]
    temp = np.pow(10, rng.normal(4, 0.1, size=mesh_shape), dtype=np.float64)
    clump = np.ones_like(temp)

    # Define some random sources
    src_pos = rng.integers(0, mesh_size, size=(3, num_sources), dtype=np.int32)
    src_flux = np.pow(10, rng.uniform(2, 10, size=num_sources), dtype=np.float64)

    # Size of a cell
    dr = (5 * u.Mpc / mesh_size).cgs.value
